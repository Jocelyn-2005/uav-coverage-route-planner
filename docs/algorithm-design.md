# 几何与优化算法设计

## 1. 分层求解框架

规划器采用“先几何可行、再组合优化、最后连续验证”的分层结构：

```text
语义地图 + 用户区域 + 相机/飞行参数
                ↓
有效搜索区域与飞行障碍构造
                ↓
候选扫描方向和覆盖模式生成
                ↓
覆盖航线裁剪、补漏和端点化
                ↓
航线方向与访问顺序选择
                ↓
可见图障碍最短路连接
                ↓
连续摄影采样与覆盖复核
                ↓
均匀飞控途径点 + JSON/YAML 导出
```

这样做的原因是连续多边形覆盖、航线集合选择、访问排序和障碍最短路耦合后属于高复杂度组合优化问题。分层方法容易解释、可重复、可单元测试，也允许以后单独替换某一层求解器。

## 2. 有效搜索区域

首先将用户搜索多边形裁剪到语义地图边界，再扣除建筑占地和显式排除区。建筑占地永远不属于地面摄影搜索区域；但建筑是否为飞行障碍取决于无人机能否在满足垂直净空时越楼。

这里区分两个容易混淆的集合：

- 地面搜索区域：相机需要覆盖的地面；
- 飞行自由空间：无人机平面投影允许经过的区域。

高楼可能同时从两个集合中排除；低楼占地不需要搜索，但无人机可能从其上空越过。

## 3. 扫描线候选生成

### 3.1 往复式覆盖

将有效区域旋转到候选扫描方向，在旋转空间生成等间距平行线，与搜索多边形求交后得到若干直线段。相邻扫描线交替反向，形成 boustrophedon（牛耕式/往复式）基础顺序。

扫描线间距由 footprint 宽度和旁向重叠率决定。每条交线最初会生成密集摄影样本，用于覆盖验证；进入飞行路由前再压缩为有效直线段的首尾端点。

### 3.2 轮廓覆盖

以任务中心为基准生成由内向外的同心缓冲边界，与有效区域相交后得到轮廓航段。该模式适合中心出发或近似环形区域，但在矩形建筑密集园区中往往产生更多弯折。

### 3.3 扫描方向候选

当前候选角度为 $[0,180)$ 内每 15° 一个方向。基础评分包括候选点路径长度、跨航线空驶、转弯数、航段数和航点数。若配置显式给出角度，则不执行角度搜索。

更严格的研究版本应让每个候选角度走完“航线排序 + 避障 + 连续覆盖”全流程，再用真实任务成本评分。

## 4. Patch 覆盖与补漏

有效区域被划分为 patch。对所有采样 footprint 求几何并集，然后计算每个 patch 的面积覆盖率。未达到阈值的 patch 会产生补漏候选。

当前补漏算法使用 patch 内代表点，并按欧氏增量执行 cheapest insertion。它简单稳定，但没有把多个相邻未覆盖 patch 聚类为一条补漏航线，也没有在插入阶段使用真实障碍距离。这是后续优化的重要方向。

推荐演进方案：

1. 对相邻未覆盖 patch 做连通分量聚类；
2. 优先延伸已有航线；
3. 无法延伸时为每个聚类生成短补漏 lane；
4. 将补漏 lane 与主航线一起进行全局排序。

## 5. Lane-first 路由

旧式方案把每个摄影样本都当作必须访问的航点，导致同一条直线上反复进行障碍最短路。当前方案先把密集样本恢复为 coverage lane：

- 按 `scan_line_index` 和 `scan_segment_index` 分组；
- 使用建筑安全缓冲精确裁剪直线段；
- 每个可行片段只保留首尾端点；
- 独立补漏点作为单点任务保留。

这使组合优化对象从“数百个拍照点”降为“数十条可定向航线”。

## 6. 航线方向与访问顺序

每条双端点航线都有正向和反向两种状态。当前算法从起点开始，枚举所有未访问航线的两个入口，计算当前位置到入口的障碍最短路距离，选择代价最小者，然后从该航线出口继续。

这是 nearest-neighbor 型确定性贪心，优点是速度快、结果稳定；缺点是只看一步，可能产生后续跨区折返，不保证全局最优。

建议后续加入：

- 2-opt：交换两段访问顺序并同时重新选择航线方向；
- Or-opt：移动一个或多个连续航线块；
- simulated annealing / tabu search：跳出局部最优；
- 小规模精确 GTSP/MILP：作为启发式结果的最优性对照；
- 大规模 LKH/OR-Tools 路由：提供更强基线。

任何后优化都必须重新执行障碍连接与覆盖验证，不能只在欧氏距离矩阵上优化。

## 7. 可见图避障

对所有障碍安全缓冲多边形的外边界顶点建立可见图：若两顶点之间的线段不穿过障碍内部，就连接一条以欧氏长度为权重的边。查询时把起点和终点接入复用图，再求最短路径。

复用静态顶点图避免为每一对航线端点重复构建完整图。输出路径为障碍角点折线，满足二维几何安全，但还存在两个工程问题：

- 折点可能具有很大的航向突变；
- 路径紧贴安全缓冲边界，对定位误差的鲁棒性取决于净空设置。

后续可以在安全可行前提下进行 line-of-sight shortcut，并加入 Dubins 曲线、圆弧倒角或最小转弯半径约束。

## 8. 连续摄影与机会式采图

覆盖航段默认开启固定频率摄影。连接、避障和返航段默认不承担搜索任务；如果某个转场 footprint 能覆盖尚未覆盖的有效地面，则该段可被标记为 `capture_enabled=true`，进行机会式采图。

这样既避免所有转场盲目拍摄，又利用不可避免的飞行补充狭碎区域覆盖。最终覆盖率以实际启用摄影的航段采样为准。

## 9. 飞控途径点生成

几何规划只需要端点和转折点，但飞控通常需要更密集的参考轨迹。系统对每个直线航段独立均匀细分，确保相邻途径点不超过配置间距。

细分不会改变航线形状、航段类型或总长度。每个子航段继承原航段的：

- `kind`；
- `heading_deg`；
- `speed_mps`；
- `capture_enabled`；
- 扫描线来源标识。

## 10. 复杂度与可复现实验

若障碍可见图有 $V$ 个顶点，朴素建图需要 $O(V^2)$ 次可见性判断。设 coverage lane 数为 $K$，贪心排序最多进行 $O(K^2)$ 次入口代价查询。Patch 覆盖评估的实际开销取决于 patch 数、footprint 数和 Shapely 几何求交。

算法所有候选顺序和并列规则均确定，因此相同输入应得到相同输出。科研实验建议记录：Git commit、输入文件哈希、配置 YAML、运行时间、覆盖率、总航程、非作业航程、转弯数、不可达面积和规划器版本。

## 11. 当前研究缺口

1. 贪心 lane 排序会出现局部折返；
2. 补漏点尚未聚类成补漏航线；
3. 没有最小转弯半径和动力学可行性约束；
4. 扫描方向基础评分尚未使用完整飞行成本；
5. 没有给出启发式解相对最优下界的 gap；
6. 固定高度模型未处理地形、GSD 和三维遮挡。

这些缺口可以自然转化为课程设计、毕业设计或论文中的对照实验与算法改进项。
