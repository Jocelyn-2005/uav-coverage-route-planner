...
几何生成器与组合优化器是两个独立层次。当前正式实现两个平行的基础生成器：

1. `ScanlineClippedGenerator`：先在整个有效区域生成全局平行扫描线，再由边界和
   障碍裁剪为 lane；
...
lane”两种基础几何视角。两者都转换成相同的 `LaneRoutingProblem`，共享排序算法、
...
连续遮挡复核后若总体覆盖率不足，系统会再执行最多两轮补漏和障碍重路由；仍低于阈值时输出 `infeasible_coverage`，不会将其标记为可执行任务。
...
覆盖航段默认计入连续视频检测。规划器沿航段按 `analysis_rate_hz` 离散积分相机视锥扫过的地面；这只是计算精度参数，不代表相机定频拍照。连接、避障和返航段默认不承担搜索任务；若转场视野能补充本责任区未覆盖地面，该段可标记为 `detection_enabled=true`。
